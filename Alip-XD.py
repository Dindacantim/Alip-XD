...
def login_lagi334():
	try:
		...
		print(f'  {x}[{h}•{x}]{h} LOGIN BERHASIL.........Masukin lagi Alip-XD.py{x} ');time.sleep(1)
		exit()
	except Exception as e:
		os.system("rm -f .token.txt")
		os.system("rm -f .cok.txt")
		print(f'  %s[%sx%s]%s LOGIN GAGAL.....CEK TUMBAL LU !!%s'%(x,k,x,m,x))
		exit()
def bot():
	try:
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'├──>{m}•{k}•{h}•{x} Proses Crack Mohon Bersabar {m}•{k}•{h}•{x} ')
	print('|')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r[{h}!{x}] Yakin IJO ? {h}{P}{b}{loop}{P}/{u}{len(id)}{P} {H}OK : {P}{ok}{P} {k}CP : {P}{cp}{x}  {bo}{'{:.0%}'.format(loop/float(len(id)))}{P}  "),
	sys.stdout.flush()
